...
[English](./README_EN.md) · [日本語](./README_JA.md) · [维护手册](./docs/MAINTENANCE.md)
...
约 80 条 N5–N2 语法语料，**查询改写 → 并发混合召回 → RRF 融合 → LLM 精排** 全链路本地化，每个旋钮都有可量化的优化前后对比：
...
50 题黄金集（含大量口语化提问）+ 10 题离题对抗集，embedding `qwen3-embedding:0.6b`：

| 检索模式 | recall@1 | recall@5 | MRR | NDCG@10 |
| --- | --- | --- | --- | --- |
| BM25（纯关键词） | 34/50 | 44/50 | 0.766 | 0.799 |
| Vector（纯向量） | 42/50 | 48/50 | 0.887 | 0.910 |
| Hybrid（RRF 融合） | 42/50 | 48/50 | 0.881 | 0.905 |
| **Hybrid + Rerank（三段式）** | **48/50** | **49/50** | **0.967** | **0.970** |

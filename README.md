...
| --- | --- | --- | --- | --- |
| BM25（纯关键词） | 43/65 | 56/65 | 0.755 | 0.793 |
| Vector（纯向量） | 49/65 | 63/65 | 0.839 | 0.872 |
| Hybrid（RRF 融合） | 53/65 | 63/65 | **0.877** | 0.904 |
| **Hybrid + Rerank（三段式）** | **60/65** | **64/65** | **0.951** | **0.960** |

**语料扩容回归分析**：语料从 80 → 177 条（2.2×）后，纯向量 MRR 从 0.887 跌至 0.839——新增的「辨析」类条目与原条目语义高度相近，干扰了向量近邻；而 Hybrid 基本稳住并**反超纯向量**（80 条时 hybrid 0.881 < vector 0.887；177 条时 hybrid 0.877 > vector 0.839）。混合检索的鲁棒性正是在更大、更密的语料上才显现——这也是小规模 demo 测不出来的工程结论。

...
- 每个浏览器自动获得独立访客身份，注册时原地升级并保留已有学习数据
...
| **用户系统** | 注册 / 登录（scrypt 密码哈希 + HMAC token） · 独立访客身份与无损账号升级 · 注册/登录/Agent 分级限流 · Cloudflare Turnstile 防 Bot 注册 |
...
启动时控制台会打印 `[payments] OKX provider 已启用`、`Alipay provider` 或 `Mock provider`，便于排错。

OKX 使用只读 Funding API 查询充值地址与充值记录。浏览器只会收到公开充值地址，API Key、Secret 和 Passphrase 始终留在 Render 服务端：
...
用户按指定网络充值准确金额并提交 TxID。后端只有在 OKX 返回 `Deposit successful`，且 TxID、币种、网络、金额、地址和订单时间全部匹配后才授予权益；同一 TxID 不能重复使用。API Key 只需要“读取”权限，不应开启交易或提现权限。
...
| 用户数据 | Supabase PostgreSQL Shared Pooler | 用户、访客、SRS、Agent Memory、run/task 和权益均按 `user_id` 持久化隔离 |
...
后续请求带 `Authorization: Bearer <TOKEN>`，记忆卡 / 练习 / Agent 记忆 / run/task / 权益即按该身份隔离。前端首次访问会自动领取独立访客 Token，注册时原地升级为正式账号并保留数据。生产环境还要求前端提交有效的 Turnstile token，并设置强随机 `JVM_AUTH_SECRET`、`DATABASE_URL`、`TURNSTILE_SITE_KEY` 和 `TURNSTILE_SECRET_KEY`。
...
- [x] 独立访客身份、注册原地升级、Agent run/task 按账户持久化
- [x] 访客创建、注册、登录和 Agent 高成本接口分级限流

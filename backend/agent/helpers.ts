...
import { extractJapaneseTerms } from '../learningSubagents';
import { AGENT_MEMORY_TYPES, getMemorySettings } from '../db';
import { conjugate } from '../conjugationEngine';
import { getTokenizer } from '../tokenizer';
import { getSceneIdsForVerb, getVerbsForScene } from '../sceneData';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// 读取动词库（buildVerbSimilarWords 依赖）
const commonVerbs: any = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'common-verbs.json'), 'utf8'));

export const formLabelMap: Record<string, string> = {
  negative: '否定式',
  polite: '礼貌式',
  teForm: 'て形',
  taForm: '过去式',
...
// 自动检测动词类型
export function detectVerbType(verb: string): string | null {
  ...
  let verbToken = tokens.slice().reverse().find((t: any) => t.pos === '動詞');
...
export function buildPracticeProfile(records: any[]): any {
  ...
  const totalCorrect = records.filter((record: any) => record.isCorrect).length;
  const totalDuration = records.reduce((sum: number, record: any) => sum + (record.durationMs || 0), 0);
  ...
  profile.todayAttempts = records.filter((record: any) => getDateKey(record.answeredAt) === todayKey).length;
  ...
  profile.weakestForms = Array.from(formStats.values())
    .map((item: any) => ({ ...item, accuracy: Math.round((item.correct / item.attempts) * 100) }))
    .sort((a: any, b: any) => a.accuracy - b.accuracy || b.attempts - a.attempts)
    .slice(0, 3);
  ...
  profile.wrongBook = Array.from(wrongBookMap.values())
    .sort((a: any, b: any) => b.wrongCount - a.wrongCount || new Date(b.lastAnsweredAt).getTime() - new Date(a.lastAnsweredAt).getTime())
    .slice(0, 8);
...
export function buildVerbSimilarWords(lookup: any, limit: number = 8): any[] {
  const sceneIds = getSceneIdsForVerb(lookup.word);
  const sameScene = sceneIds.flatMap((id: any) => getVerbsForScene(commonVerbs, id));
  ...
  for (const item of source) {
    ...
    if (sceneIds.some((id: any) => getSceneIdsForVerb(item.kanji).includes(id))) {
  ...
  const seen = new Set();
  return [...sameType, ...others].filter((item: any) => {
...
export function memoryCandidatesFromToolResult(toolName: string, result: any): any[] {
  ...
  if (toolName === 'recommend_similar' && Array.isArray(result)) {
    return result.slice(0, 6).map((item: any) => ({
...
export function dedupeMemoryCandidates(candidates: any[] = []): any[] {
  const seen = new Set();
...
export function buildFallbackTutorAnswer(message: string, toolCalls: any[] = []): string {
  const lookupSummaries = toolCalls
    .filter((call: any) => call.name === 'lookup_word')
    .map((call: any) => {
      const word = call.arguments?.word || '目标词';
      return `- **${word}**：${call.result || '未找到明确词典结果。'}`;
    })
    .join('\n');
  const usedTools = toolCalls.map((call: any) => `\`${call.name}\``).join('、') || '本地规则';
...
export function normalizeJlptLevel(raw: any = ''): string {
...
export function buildDifficultyInstruction(level: string = 'N3'): string {
...
export function normalizeAgentExamples(items: any[] = []): any[] {
  return items
    .filter((item: any) => item && (item.japanese || item.kana || item.chinese))
    .map((item: any) => ({
      ...
      components: Array.isArray(item.components)
        ? item.components
          ...
          .filter((part: any) => part.label && part.text)
        : []
    }))
    .filter((item: any) => item.japanese && item.chinese)
    .map((item: any) => ({
      ...item,
...
export function buildFallbackAgentExamples({ message, memoryCandidates = [], difficultyLevel = 'N3' }: any): any[] {
  const source = memoryCandidates.length > 0
    ? memoryCandidates
    : extractJapaneseTerms(message).slice(0, 3).map((word: string) => ({ word, reading: '' }));

  const examples = source.slice(0, 3).map((item: any, index: number) => {
    const word = item.word || 'この表現';
    const reading = item.reading || word;
    const templatesByDifficulty: Record<string, any[]> = {
      N5: [
        {
...
export function pickPracticeForm(message: string = ''): any {
  ...
  const matched = formCandidates.find((item: any) => item.tests.some((test: RegExp) => test.test(message)));
  return matched || { key: 'teForm', label: formLabelMap.teForm };
}

export function inferMarkedRole(text: string = '', marker: string = ''): string {
  const trimmed = String(text || '').trim();
...
export function inferPredicateStart(tokens: any[] = []): number {
...
export function annotateSentenceComponents(sentence: string = ''): any[] {
  if (!getTokenizer() || !sentence) return [];

  try {
    const tokens = getTokenizer().tokenize(sentence).filter((token: any) => token?.surface_form);
    if (tokens.length === 0) return [];

    const predicateStart = inferPredicateStart(tokens);
    const components: any[] = [];
    const appendComponent = (label: string, text: string) => {
    ...
    for (let index = 0; index < boundary; index += 1) {
      ...
      if (token.pos === '助詞' && ['は', 'も', 'が', 'を', 'に', 'へ', 'で', 'と', 'から', 'まで', 'より'].includes(token.surface_form)) {
        ...
        const phrase = phraseTokens.map((item: any) => item.surface_form).join('');
    ...
    if (buffer.length > 0) {
      appendComponent('补足', buffer.map((item: any) => item.surface_form).join(''));
    }

    if (predicateStart >= 0) {
      appendComponent('谓语', tokens.slice(predicateStart).map((item: any) => item.surface_form).join(''));
    }
...
export function shouldOfferInteractivePractice({ message = '', context = {}, memoryCandidates = [] }: any = {}): boolean {
  ...
  if (memoryCandidates.some((item: any) => item.wordType === 'verb' || detectVerbType(item.reading || item.word || ''))) return true;
  if ((context.intent?.terms || []).some((term: string) => detectVerbType(term))) return true;
  return extractJapaneseTerms(message).some((term: string) => detectVerbType(term));
}

export function buildInteractivePractice({ message = '', intent = {}, context = {}, memoryCandidates = [] }: any = {}): any {
  ...
  const candidateWord = sourceLookup?.dictionaryForm
    ...
    || sourceLookup?.word
    || memoryCandidates.find((item: any) => detectVerbType(item.reading || item.word))?.reading
    || intent.terms?.find((term: string) => detectVerbType(term))
    || null;
  ...
  try {
    ...
    const answer = (conjugation as any)[form.key];
...
export function buildFallbackFollowUpQuestions({ intent = {}, context = {}, memoryCandidates = [] }: any = {}): string[] {
...
export function parseJsonStringList(text: string = ''): string[] | null {
...
export function formatSuggestionConversation(conversation: any[] = []): string {
  return conversation
    .map((item: any) => {
...
export function compactConversationTurns(conversation: any[] = [], keepTail: number = 4): any {
  ...
  const normalized = safeConversation
    ...
    .map((item: any) => ({ ...item, content: item.content.slice(0, 320) }));
...
export function isCompactFocusWord(word: string = ''): boolean {
...
export function normalizeThreadTopic(topic: string = ''): string {
  return String(topic || '')
    .replace(/\s+/g, '')
    .replace(/[.…]{2,}$/g, '')
    .trim();
...
export async function buildThreadSummary(userStore: any, { userId, currentRunId = '', threadId = '', limit = 10 }: any = {}): Promise<any> {
  const runs = (await userStore.listAgentRuns({ userId, threadId, limit: limit + 2 }))
    .filter((run: any) => run.runId !== currentRunId)
    .filter((run: any) => ['completed', 'cancelled', 'failed'].includes(run.status))
    .slice(0, limit);

  const seenTopics = new Set();
  const topics = runs
    .map((run: any) => run.title || buildAgentRunTitle(run.question || ''))
    .filter(Boolean)
    .filter((topic: string) => {
  ...
  const focusWords = ([...new Set(runs.flatMap((run: any) => run.metadata?.compactEntry?.focusWords || []))] as string[])
    .filter(isCompactFocusWord)
    .slice(0, 12);

  const practiceFocuses = runs
    .map((run: any) => run.metadata?.compactEntry?.practiceFocus || '')
    .filter(Boolean)
    .filter((item: string, index: number, source: string[]) => source.indexOf(item) === index)
    .slice(0, 6);
...
export function buildRunCompactEntry({ message = '', intent = {}, context = {}, finalState = {} }: any = {}): any {
  const focusWords = [
    context.lookup?.word,
    ...(Array.isArray(finalState.memoryCandidates) ? finalState.memoryCandidates.map((item: any) => item.word) : [])
  ]
    .map((item: any) => String(item || '').trim())
    .filter(isCompactFocusWord)
    .filter((item: string, index: number, source: string[]) => source.indexOf(item) === index)
  ...
  const exampleHighlights = Array.isArray(finalState.structuredExamples)
    ? finalState.structuredExamples
      .map((item: any) => String(item?.japanese || '').trim())
      .filter(Boolean)
      .slice(0, 2)
    : [];
...
export async function buildPersistedCompactSummary(userStore: any, { userId, currentRunId = '', threadId = '', conversation = [], model = '' }: any = {}): Promise<any> {
...
export async function generateFollowUpQuestions({ message = '', finalAnswer = '', intent = {}, context = {}, memoryCandidates = [] }: any = {}): Promise<string[]> {
  ...
  try {
    ...
    const normalized = [...new Set(items
      .map((item: string) => String(item || '').replace(/\s+/g, ' ').trim())
      .filter(Boolean)
    )].slice(0, 3);
...
export async function extractAgentMemoryCandidates({ message = '', finalAnswer = '' }: any = {}): Promise<any[]> {
  ...
  try {
    ...
    return parsed
      .filter((item: any) => item && AGENT_MEMORY_TYPES.includes(String(item.type)) && item.mkey && item.value)
      .map((item: any) => ({
...
export function buildPracticeOptions(conjugation: any, answerKey: string, answer: string, difficultyLevel: string = 'N3'): string[] {
...
export async function generateStructuredAgentExamples({ message, finalAnswer, toolCalls, memoryCandidates = [], context = {} }: any): Promise<any[]> {

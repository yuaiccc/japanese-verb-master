...
import { findWord, findSimilarWords, getMemorySettings, AGENT_MEMORY_TYPES } from '../db.js';
import { DEFAULT_USER_ID } from '../auth.js';
import { traceLangSmithRun } from '../tracing/langsmith.js';
import { buildVerbSimilarWords, buildPracticeProfile } from './helpers.js';

// 运行时实例（由 server.js 启动时通过 configureAgentTools 注入）
let _knowledgeRetriever = null;
let _knowledgeRewriter = null;
let _userRequestStore = null;
let _userStore = null;

export function configureAgentTools({ knowledgeRetriever, knowledgeRewriter, userRequestStore, userStore }) {
...
export async function searchMediaWiki(query, project) {
  try {
...
export function searchJisho(keyword, verbOnly = true) {
...
// 查询单个词的详细信息（用于非动词查词）
export function lookupWordJisho(keyword) {
  return new Promise((resolve, reject) => {
...
export async function externalJapaneseSearch(query) {

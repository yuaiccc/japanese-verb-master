...
export function ensureAuthSchema(db: any): void {
  db.exec(`
...
function signToken(userId: number, { guest = false }: { guest?: boolean } = {}): string {
...
function verifyToken(token: string): TokenIdentity | null {
...
export function authOptional(req: any, _res: any, next: NextFunction): void {
...
export function authRequired(req: any, res: any, next: NextFunction): void {
  const header = req.headers?.authorization || '';
  const token = header.startsWith('Bearer ') ? header.slice(7) : '';
  const identity = verifyToken(token);
...
export function identityRequired(req: any, res: any, next: NextFunction): void {

...
function fetchPage(keyword: string, page: number): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    https.get(url, { timeout: 15000 }, (res) => {
      ...
      .on('timeout', function(this: any) { this.destroy(); reject(new Error('Request timeout')); });
  });
}

function parseJishoItem(item: any): any {
...
async function fetchLevel(level: string): Promise<any[]> {

...
function buildMessages(query: string, chunks: any[], { forceCitation, abstain }: any): any[] {
  const ctx = chunks.map((c: any, i: number) => `[${i + 1}] ${c.title}\n${String(c.content || '').replace(/\s+/g, ' ')}`).join('\n\n');
...
function isAbstainText(text: string): boolean {
  ...
  return {
    async answer(query: string): Promise<any> {
      ...
      const cited = [...String(text).matchAll(/\[(\d+)\]/g)]
        ...
        .filter((n: number) => n >= 1 && n <= results.length);

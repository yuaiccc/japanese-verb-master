...
export function scoreCase(hits: any[], expected: any): any {
  const accepted = new Set(Array.isArray(expected) ? expected : [expected]);
  const index = hits.findIndex((h: any) => accepted.has(`${h.docId}::${h.title}`));
...
export function summarize(rows: any[]): any {
  ...
  const atK = (k: number) => `${rows.filter((r: any) => r.rank !== null && r.rank <= k).length}/${total}`;
  const mrr = rows.reduce((sum: number, r: any) => sum + r.rr, 0) / Math.max(total, 1);
  // golden set 每题单一相关条目 ⇒ IDCG@k = 1（相关项在 rank1 时 DCG=1）。
  // 故 NDCG@k 退化为 命中 k 内时 1/log2(rank+1)，否则 0。指标名与行业一致，便于对比。
  const ndcgAt = (k: number) => rows.reduce(
    (sum: number, r: any) => sum + (r.rank !== null && r.rank <= k ? 1 / Math.log2(r.rank + 1) : 0),
    0
  ) / Math.max(total, 1);
  return {
    'recall@1': atK(1),
...
export async function runEval({ retrieverFactory, cases, topK = 10, withRerank = false }: any): Promise<any> {
  const output: any = {};
  const run = async (mode: string, queryOpts: any = {}) => {

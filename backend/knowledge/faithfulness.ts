...
export function parseClaims(text: string): any[] | null {
  if (!text) return null;
...
export function createFaithfulnessJudge({ chatFn }: any = {}): any {
  return {
    // 返回单题指标；abstained 的答案没有陈述，记为零陈述（既不贡献覆盖率也不贡献幻觉）。
    async judge({ query, answer, contextChunks = [], abstained = false }: any): Promise<any> {
      if (abstained) {
        return { total: 0, supported: 0, cited: 0, claims: [], abstained: true };
      }
      const ctx = contextChunks
        .map((c: any, i: number) => `[${i + 1}] ${c.title}: ${String(c.content || '').replace(/\s+/g, ' ')}`)
        .join('\n\n');
      let text = '';
      try {
        text = await chatFn({
      ...
      return {
        total: claims.length,
        supported: claims.filter((c: any) => c.supported).length,
        cited: claims.filter((c: any) => c.cited).length,
        claims
      };
    }
  };
...
// 把多题的单题结果聚合成报告。
export function summarizeFaithfulness(rows: any[]): any {
  const answered = rows.filter((r: any) => !r.abstained && !r.judgeError && !r.parseError);
  const abstained = rows.filter((r: any) => r.abstained).length;
  const totalClaims = answered.reduce((s: number, r: any) => s + r.total, 0);
  const supported = answered.reduce((s: number, r: any) => s + r.supported, 0);
  const cited = answered.reduce((s: number, r: any) => s + r.cited, 0);

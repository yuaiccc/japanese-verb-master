...
import { ensureVecTable } from './schema';

const EMBED_BATCH = 16;

export async function ingestKnowledge({ db, embedder, sourceDir }: any): Promise<any> {
  const report: any = { added: 0, updated: 0, removed: 0, unchanged: 0, embedded: 0, failed: [] };
  const files = fs.readdirSync(sourceDir).filter((f: string) => f.endsWith('.md'));
  const incoming = files.flatMap((f: string) => parseSourceFile(fs.readFileSync(path.join(sourceDir, f), 'utf8'), f));

  const existing = new Map<string, any>(
    db.prepare('SELECT id, doc_id, title, content_hash, has_embedding FROM knowledge_chunks').all()
      .map((row: any) => [`${row.doc_id} ${row.title}`, row])
  );
  ...
  if (embedder && toEmbed.length > 0) {
    ...
    let deleteVec: any = null;
    let insertVec: any = null;
    for (let i = 0; i < toEmbed.length; i += EMBED_BATCH) {
      const batch = toEmbed.slice(i, i + EMBED_BATCH);
      try {
        const vectors = await embedder.embed(batch.map((item: any) => item.text));
        ...
        if (!ensureVecTable(db, dim)) {
          for (let j = i; j < toEmbed.length; j += EMBED_BATCH) {
            ...
            report.failed.push({ ids: remainingBatch.map((b: any) => b.id), error: 'ensureVecTable failed' });
          }
          break;
        }
        ...
        const writeVecBatch = db.transaction(() => {
          batch.forEach((item: any, j: number) => {
        ...
        report.failed.push({ ids: batch.map((b: any) => b.id), error: error.message });
      }
    }
  }
  return report;

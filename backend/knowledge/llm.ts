...
export function createKnowledgeChat({ fetchImpl = fetch }: any = {}): any {
  return async function chat({ messages, model, temperature = 0, maxTokens = 800, responseFormat, timeoutMs = 45000 }: any): Promise<string> {
    ...
    if (provider === 'ollama') {
      const ollama: any = settings.baseUrl ? new Ollama({ host: settings.baseUrl }) : ollamaClient;
    ...
    const body: any = { model: targetModel, messages, stream: false, temperature, max_tokens: maxTokens };

...
export function createReranker({ chatFn, model, candidateLimit = 10, temperature = 0 } = {}) {
  return {
    enabled: !!chatFn,
    async rerank(query, candidates, { topK = 5 } = {}) {
      ...
      const reordered = order.map(i => pool[i]);
      const used = new Set(order);
      pool.forEach((c, i) => { if (!used.has(i)) reordered.push(c); }); // 补回 LLM 漏排的候选

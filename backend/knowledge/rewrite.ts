...
function parseRewrite(text: string): any {
  if (!text) return null;
  ...
  try {
    ...
    const terms = Array.isArray(parsed.terms)
      ? parsed.terms.filter((t: any) => typeof t === 'string' && t.trim()).map((t: string) => t.trim())
      : [];
...
export function createQueryRewriter({ chatFn, model, temperature = 0 }: any = {}): any {
  const passthrough = (original: string) => ({ query: original, rewritten: original, terms: [], changed: false });
  return {
    enabled: !!chatFn,
    async rewrite(query: string): Promise<any> {

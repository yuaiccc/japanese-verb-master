...
export function registerDictionaryRoutes(app: any, deps: Record<string, any>): void {
  const {
    agentHelpers,
    conjugate,
    findWord,
  ...
  app.post('/api/furigana', express.json(), (req: any, res: any) => {
    ...
    const results = texts.map((t: string) => generateFuriganaHtml(t));
    res.json({ results });
  });

  // 获取可用模型
  app.get('/api/ai-models', async (req: any, res: any) => {
    ...
    try {
      ...
      res.json(response.models.map((m: any) => m.name));
    } catch (error) {
  ...
  // 场景列表 API
  app.get('/api/scenes', (req: any, res: any) => {
    res.json(sceneCatalog);
  });

  // 练习画像 API
  app.get('/api/practice-profile', async (req: any, res: any) => {
    try {
  ...
  // 练习记录写入 API
  app.post('/api/practice-records', async (req: any, res: any) => {
    try {
      const {
        verb,
        formKey,
  ...
  app.post('/api/ai-explain', async (req: any, res: any) => {
    try {
      ...
      if (wordType && wordType !== 'verb') {
        ...
        const wordTypeNames: Record<string, string> = {
        ...
        const meaningsStr = wordInfo?.meanings
          ? wordInfo.meanings.map((m: any, i: number) => `${i + 1}. [${m.pos}] ${m.definitions}`).join('\n')
          : '';
...
6. 动词类型请使用中国通用术语：五段动词、一段动词、サ变动词、カ变动词，不要用 Godan、Ichidan 等英文。`;
  ...
  app.get('/api/suggest', async (req: any, res: any) => {
    try {
      ...
      if (queryHira && localSuggestions.length < 8) {
        ...
        const seen = new Set(localSuggestions.map((w: any) => w.kanji + w.kana));
      ...
      if (remoteOnly.length > 0) {
        try {
          bulkInsert(remoteOnly.map((item: any) => ({
            kanji: item.kanji,
            kana: item.kana,
            romaji: item.romaji,
  ...
  app.get('/api/conjugate', async (req: any, res: any) => {
    try {
            ...
            if (wordInfo && wordInfo.wordType !== 'other') {
              ...
              try {
                bulkInsert([{
                  ...
                  meaning: wordInfo.meanings.map((m: any) => m.definitions).join('; '),
      ...
      const matchedVerb = dbWord || commonVerbs.find((v: any) =>
  ...
  // 获取支持的动词类型
  app.get('/api/verb-types', (req: any, res: any) => {
    res.json({
      types: [
        {
          id: 'GODAN',
  ...
  // Dojo (动词变形道场) 题库 API
  app.get('/api/dojo-quiz', async (req: any, res: any) => {
    try {

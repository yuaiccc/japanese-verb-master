...
export function registerMemoryRoutes(app: any, deps: Record<string, any>): void {
  const {
    userStore,
    getMemorySettings,
    saveMemorySettings,
  ...
  async function recordAgentPracticeToMemory({ question, userAnswer, isCorrect, hintUsed }: any, userId: any = DEFAULT_USER_ID): Promise<any> {
  ...
  app.post('/api/dojo-agent-turn', async (req: any, res: any) => {
    try {
      const {
        question = {},
        userAnswer = '',
  ...
  // 记忆卡片 API：间隔复习队列
  app.get('/api/memory-cards', async (req: any, res: any) => {
    try {
  ...
  app.post('/api/memory-cards', async (req: any, res: any) => {
    try {
      const card = req.body || {};
      if (!card.word) {
  ...
  app.delete('/api/memory-cards/:id', async (req: any, res: any) => {
    try {
  ...
  app.get('/api/memory-review-queue', async (req: any, res: any) => {
    try {
  ...
  app.post('/api/memory-cards/:id/review', async (req: any, res: any) => {
    try {
  ...
  app.get('/api/memory-settings', (req: any, res: any) => {
    res.json(getMemorySettings());
  });

  app.post('/api/memory-settings', (req: any, res: any) => {
    try {
  ...
  app.get('/api/knowledge/embedding-settings', (req: any, res: any) => {
    const { apiKey, ...rest } = getKnowledgeEmbeddingSettings(db);
    res.json({ ...rest, apiKeySet: !!apiKey });
  });
  app.post('/api/knowledge/embedding-settings', (req: any, res: any) => {

...
import { parseFeishuTextMessage } from '../integrations/feishu';

export function registerSystemRoutes(app: any, deps: Record<string, any>): void {
  const {
    registerLimiter,
    guestLimiter,
    loginLimiter,
  ...
  // 注册：用户名 + 密码，scrypt 哈希
  app.get('/api/auth/captcha-config', (_req: any, res: any) => {
    res.json(getTurnstileConfig());
  });

  app.post('/api/auth/guest', guestLimiter, async (req: any, res: any) => {
  ...
  app.get('/api/entitlements', async (req: any, res: any) => {
    try {
      const entitlements: any = {};
      for (const def of Object.values(SKUS)) {
        entitlements[(def as any).entitlement] = await userStore.hasEntitlement((def as any).entitlement, req.userId);
      }
  ...
  app.post('/api/payments/orders', async (req: any, res: any) => {
  ...
  app.get('/api/payments/orders/:outTradeNo', async (req: any, res: any) => {
    try {
  ...
  app.post('/api/payments/orders/:outTradeNo/txid', async (req: any, res: any) => {
  ...
  app.post('/api/payments/orders/:outTradeNo/simulate-confirm', async (req: any, res: any) => {
    try {
  ...
  app.get('/api/llm-status', (req: any, res: any) => {
  ...
  app.get('/api/hot-placeholders', async (req: any, res: any) => {
    try {
  ...
  app.get('/api/llm-settings', (req: any, res: any) => {
    res.json(getRuntimeLlmSettings());
  });

  app.post('/api/llm-settings', (req: any, res: any) => {
    try {
  ...
  app.post('/api/integrations/feishu/webhook', (req: any, res: any) => {
    ...
    processFeishuMessage(parsed, { dedupe: false }).catch((error: any) => {
      console.error('[feishu] failed to process message:', error);
      feishuClient.replyText(parsed.messageId, `处理失败：${error.message || '未知错误'}`).catch((replyError: any) => {

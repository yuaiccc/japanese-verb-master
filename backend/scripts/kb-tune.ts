...
async function evalQueries(retriever: any, queryOf: (item: any) => Promise<string> | string): Promise<any> {
  const rows: any[] = [];
...
for (const k of [10, 20, 40, 60, 100, 200]) {
  ...
  kSweep[`k=${k}`] = await evalQueries(retriever, (item: any) => item.query);
}
...
if (process.env.KB_TUNE_REWRITE === '1') {
  ...
  const compare: any = {
    '改写关闭': await evalQueries(retriever, (item: any) => item.query),
    '改写开启': await evalQueries(retriever, async (item: any) => {

...
import { ensureAuthSchema, hashPassword, verifyPassword, signToken, verifyToken } from '../auth.js';
import { ensurePaymentSchema, hasEntitlement } from '../payments/provider.js';
...
// 1. 迁移：列存在
const memCols = db.prepare('PRAGMA table_info(memory_cards)').all().map(c => c.name);
const prCols = db.prepare('PRAGMA table_info(practice_records)').all().map(c => c.name);
...
const entCols = db.prepare('PRAGMA table_info(entitlements)').all().map(c => c.name);
...
const aCards = listMemoryCards(500, U_A).map(c => c.word);
const bCards = listMemoryCards(500, U_B).map(c => c.word);

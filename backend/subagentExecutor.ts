...
import { formatSpecialistNote, serializeSpecialistBrief } from './subagentContexts';
import {
  SubagentTaskStatus,
...
export class SubagentExecutor {
  ...
  acquireSandbox(state: any): any {
    return this.provider.acquire(this.subagentId, { context: state.context || {} });
  }

  emitSandboxNote(sandbox: any, title: string, content: string): void {
  ...
  async withSandboxTimeout(sandbox: any, task: any): Promise<any> {
  ...
  async runManagedTask({ title, sandbox, run, onStatus }: any): Promise<any> {
  ...
  async runSpecialist({
    ...
    const statusEmitter = (type: string, task: any) => {
  ...
  async runToolSubagent({
    ...
    this.emitSandboxNote(
      sandbox,
      title,
      `${title} 已进入独立沙盒，允许工具：${plannedTools.map((tool: any) => tool.name).join('、') || '无'}`
    );
    ...
    try {
      const basePatch = await this.runManagedTask({
        ...
        onStatus: statusEmitter,
        run: async (task: any) => {
          await this.withSandboxTimeout(sandbox, async (signal: any) => {
      ...
      return {
        ...
        messages: [
          ...(basePatch.messages || []),
          ...toolPayloads.map((payload: any) => buildToolMessage(payload)).filter(Boolean)
        ],
        subagentContexts: {
          ...state.subagentContexts,
          ...
          [this.subagentId]: {
            ...
            sandbox: sandbox.describe(),
            plannedTools: plannedTools.map((tool: any) => tool.name)
          }
        },
        completed: [...state.completed, this.subagentId]
      };

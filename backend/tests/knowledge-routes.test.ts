...
test('GET /api/knowledge/stats reports counts', async () => {
  const { base, server } = await makeApp();
  const data = await (await fetch(`${base}/api/knowledge/stats`)).json() as any;
...
test('chunk CRUD roundtrip schedules reindex', async () => {
  ...
  assert.equal((db.prepare("SELECT COUNT(*) c FROM knowledge_chunks WHERE doc_id='custom'").get() as any).c, 0);
  server.close();
});

...
const planFor = (message: string) => learningSubagentRegistry.researcher.planTools({
...
test('真实查词问题仍安排 lookup_word', () => {
  ...
  const lookups = plan.filter((t: any) => t.name === 'lookup_word').map((t: any) => t.arguments.word);
...
test('简单て形问题只查本地知识和目标词', () => {
  ...
  const names = plan.map((t: any) => t.name);
  const lookups = plan.filter((t: any) => t.name === 'lookup_word').map((t: any) => t.arguments.word);
...
test('普通查词仍可推荐相似词', () => {
  const names = planFor('查一下 食べる').map((t: any) => t.name);
...
test('复习相关问题才让 Researcher 读取 memory_status', () => {
  const grammarNames = planFor('食べる 的て形怎么变？').map((t: any) => t.name);
  const memoryNames = planFor('今天有哪些词需要复习？').map((t: any) => t.name);
...
test('例句和练习请求会串联两个 specialist', () => {
  ...
  assert.deepEqual(
    getAgentQueue(intent).map((item: any) => item.id),

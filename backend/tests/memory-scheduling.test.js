...
import { scheduleReview, buildReviewQueue } from '../db.js';
...
const card = (id, dueAt, reviewCount) => ({ id, dueAt, reviewCount });
...
test('buildReviewQueue: 只放到期卡，按 due 升序', () => {
  ...
  assert.deepEqual(q.map(c => c.id), [3, 1]);
});
...
test('buildReviewQueue: 新卡受 newRemaining 约束，复习卡不受影响', () => {
  ...
  const ids = q.map(c => c.id).sort();
  // 只放 1 张新卡 + 那张复习卡
  assert.equal(q.filter(c => c.reviewCount === 0).length, 1);

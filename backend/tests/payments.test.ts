...
function withOkxEnv(): () => void {
  ...
  const original: Record<string, string | undefined> = Object.fromEntries(
    names.map(name => [name, process.env[name]] as [string, string | undefined])
  );
...
test('重复确认幂等：权益只授一次', async () => {
  ...
  const rows = db.prepare('SELECT COUNT(*) AS n FROM entitlements').get() as any;
...
test('OKX provider：同一个 TxID 不能绑定多个订单', async () => {
  ...
  const originalFetch = globalThis.fetch;
  const db = freshDb();
  globalThis.fetch = async (url: any) => new Response(JSON.stringify({
    code: '0',
    msg: '',

...
test('访客数据隔离、注册升级与 Agent run 隔离', async (t) => {
  ...
  try {
    ...
    assert.deepEqual((await store.listMemoryCards(20, guestA.id)).map(card => card.word), ['__guest_a_word__']);
    ...
    assert.deepEqual((await store.listMemoryCards(20, claimed.id)).map(card => card.word), ['__guest_a_word__']);

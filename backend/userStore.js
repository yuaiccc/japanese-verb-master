...
function normalizeMemoryCard(row) {
  if (!row) return null;
  return {
    id: Number(row.id),
    word: row.word,
...
class LocalUserStore {
  ...
  async listAgentMemory(userId) {
    return listLocalAgentMemory(userId);
  }

  async retrieveAgentMemory(userId, options) {
    ...
    const tx = db.transaction(rows => rows.forEach(row => update.run(now, row.id)));
    tx(ranked);
    return ranked;
  }

  async writeAgentMemory(candidates, userId, sourceRunId) {
    return writeLocalAgentMemory(candidates, userId, sourceRunId);
  }

  async deleteAgentMemory(id, userId) {
    return deleteLocalAgentMemory(id, userId).changes > 0;
  }

  async hasEntitlement(key, userId) {
...
class PostgresUserStore {
  ...
  async listPracticeRecords(limit = 1000, userId = 1) {
    const { rows } = await this.pool.query(`
  ...
  async listMemoryCards(limit = 500, userId = 1) {
    const { rows } = await this.pool.query(`
  ...
  async getMemoryCardByWord(word, userId = 1) {
    if (!word) return null;
    const { rows } = await this.pool.query(
  ...
  async upsertMemoryCard(card, userId = 1) {
    const now = new Date().toISOString();
    await this.pool.query(`
  ...
  async deleteMemoryCard(id, userId = 1) {
  ...
  async reviewMemoryCard(id, grade, settings, userId = 1) {
  ...
  async getDailyReviewStats(userId = 1) {
    const since = new Date();
    since.setHours(0, 0, 0, 0);
    const { rows } = await this.pool.query(`
      SELECT
  ...
  async listAgentMemory(userId = 1) {
    const { rows } = await this.pool.query(
  ...
  async retrieveAgentMemory(userId = 1, options = {}) {
    const ranked = rankAgentMemories(await this.listAgentMemory(userId), options);
    if (ranked.length > 0) {
      await this.pool.query(
        'UPDATE agent_memory SET last_used_at = NOW() WHERE user_id = $1 AND id = ANY($2::bigint[])',
        [uid(userId), ranked.map(row => row.id)]
      );
    }
    return ranked;
  }

  async writeAgentMemory(candidates = [], userId = 1, sourceRunId = '') {
  ...
  async deleteAgentMemory(id, userId = 1) {
    const result = await this.pool.query(
...
export function buildDailyQuota(stats, settings = {}) {
...
export async function buildStoreReviewQueue(store, userId, settings) {

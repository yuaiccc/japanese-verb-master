...
// === 常量 ===
const defaultAgentPlaceholderExamples: string[] = [
...
const componentHighlightClassMap: Record<string, string> = {
  主题: 'is-topic',
  主语: 'is-subject',
  宾语: 'is-object',
  谓语: 'is-predicate',
...
// === 模块级单例状态 ===
const agentThreadSummary = ref<any>(null);
const agentThreadId = ref<string>('');
const agentPlan = ref<any>(null);
const similarWords = ref<any[]>([]);
const agentLoading = ref<boolean>(false);
const agentRunning = ref<boolean>(false);
const agentInput = ref<string>('');
const agentMessages = ref<any[]>([]);
const agentRuns = ref<any[]>([]);
const activeAgentRunId = ref<any>(null);
const agentToolCalls = ref<any[]>([]);
const agentTrace = ref<any[]>([]); // 完整执行轨迹（上限 80 步），随 run 持久化，前端折叠展示
const agentMemoryCandidates = ref<any[]>([]);
const agentExamples = ref<any[]>([]);
const agentInteractivePractice = ref<any>(null);
const agentFollowUpQuestions = ref<any[]>([]);
const agentFollowUpLoading = ref<boolean>(false);
const agentUsage = ref<any>(null);
const agentPracticeInput = ref<string>('');
const agentPracticeBusy = ref<boolean>(false);
const agentPracticeHint = ref<string>('');
const agentPracticeFeedback = ref<any>(null);
const streamedAssistantText = ref<string>('');
const agentAbortController = ref<AbortController | null>(null);
const agentPlaceholderExamples = ref<string[]>([...defaultAgentPlaceholderExamples]);
const animatedAgentPlaceholder = ref<string>(agentPlaceholderExamples.value[0]);
const agentRuntimeEngine = ref<string>('LangGraph');
const agentQueue = ref<any[]>(defaultAgentQueue.map(item => ({ ...item })));
const agentRuntimeNote = ref<string>('');
const activeSubagentTaskId = ref<string>('');
const activeExampleInspector = ref<Record<string, any>>({});
const expandedCitations = ref<Set<any>>(new Set());
const activeAgentSection = ref<string>('');
const agentScrollProgress = ref<number>(0);

// === 模块级 let 变量 ===
let agentRunSeq: number = 0;
let placeholderInterval: any = null;
let placeholderRefreshInterval: any = null;
let streamRenderQueue: string = '';
let streamRenderTimer: any = null;
let streamRenderDone: boolean = false;
let streamRenderDrainResolvers: Array<() => void> = [];
let agentScrollRaf: number = 0;
...
const _deps: Record<string, any> = {
  ...
  memorySettings: ref({}),
  memoryCards: ref([]),
  normalizeMemoryCard: (card: any) => card,
...
const readLocalLlmSettings = (): Record<string, any> => {
...
const apiUrl = (path: string): string => `${import.meta.env.VITE_API_BASE || ''}${path}`;
...
const stripMarkdownInline = (text: any): string => {
  if (!text) return '';
  return String(text)
    .replace(/\|[-: ]+\|/g, ' ')
    .replace(/[|#*`~_>]+/g, ' ')
...
const compactText = (text: any, limit: number = 120): string => {
...
const parseToolPayload = (payload: any): any => {
...
const formatToolResult = (call: any = {}): string => {
...
// === 线程 / Run 管理 ===
const ensureAgentThreadId = (): string => {
...
const buildAgentRunTitle = (text: any = ''): string => {
...
const syncAgentRun = (runId: string, patch: Record<string, any> = {}): void => {
  const target = agentRuns.value.find((run: any) => run.id === runId);
  if (!target) return;
  Object.assign(target, patch);
};

const normalizePersistedAgentRun = (run: any = {}): any => ({
  id: run.runId,
...
const normalizeSubagentTaskRecord = (task: any = {}): any => ({
  taskId: task.taskId,
  runId: task.runId || '',
...
const formatSubagentTaskStatus = (status: string = ''): string => {
...
const toggleSubagentTask = (taskId: string): void => {
  activeSubagentTaskId.value = activeSubagentTaskId.value === taskId ? '' : taskId;
};

const upsertSubagentTask = (runId: string, payload: any = {}): void => {
  const target = agentRuns.value.find((run: any) => run.id === runId);
  if (!target) return;
  if (!Array.isArray(target.subagentTasks)) {
    target.subagentTasks = [];
  }
  const existingIndex = target.subagentTasks.findIndex((task: any) => task.taskId === payload.taskId);
  const record = {
...
const cancelAgentRunTasks = async (runId: string = ''): Promise<void> => {
  if (!runId) return;
  try {
...
const refreshRunTaskHistory = async (runId: string = ''): Promise<void> => {
  if (!runId) return;
  try {
...
const loadPersistedAgentRuns = async (): Promise<void> => {
  try {
    ...
    for (const run of normalized.reverse()) {
      const existingIndex = agentRuns.value.findIndex((item: any) => item.id === run.id);
      if (existingIndex >= 0) {
...
const loadAgentThreadSummary = async (currentRunId: string = ''): Promise<void> => {
  try {
    const threadId = ensureAgentThreadId();
...
// === 流式渲染器 ===
const resetStreamRenderer = (): void => {
...
const pumpStreamRenderer = (assistantMessage: any): void => {
...
const enqueueStreamText = (text: string, assistantMessage: any): void => {
  if (!text) return;
  streamRenderQueue += text;
  pumpStreamRenderer(assistantMessage);
};

const markStreamRendererDone = (): void => {
  streamRenderDone = true;
  resolveStreamDrain();
};

const waitForStreamRendererDrain = (): Promise<void> => {
...
// === Agent 运行时 ===
const resetAgentRuntime = (): void => {
  agentQueue.value = defaultAgentQueue.map(item => ({ ...item }));
  agentRuntimeNote.value = '';
  agentUsage.value = null;
};

const applyAgentQueue = (payload: any = {}): void => {
  if (Array.isArray(payload.agents)) {
    agentQueue.value = payload.agents.map((agent: any) => ({ ...agent }));
  }
  agentRuntimeNote.value = payload.note || '';
};

const upsertStreamingToolCall = (payload: any = {}, status: string = 'done'): void => {
  const existingIndex = agentToolCalls.value.findIndex((call: any) => (
...
const pushAgentEvent = ({ title, body, status = 'running' }: { title: string; body: string; status?: string }): void => {
...
const snapshotAgentTrace = (): any[] => agentTrace.value.map((step: any) => ({
  ...step,
  status: step.status === 'running' ? 'done' : step.status
}));

const parseSseFrames = (chunk: string, onEvent: (event: string, payload: any) => void): string => {
...
// === 练习 ===
const optionStateClass = (option: string): Record<string, boolean> => {
...
const requestAgentPracticeHint = async (): Promise<void> => {
...
const submitAgentPracticeAnswer = async (): Promise<void> => {
...
// === 记忆候选 ===
const addAgentMemoryCandidate = async (item: any): Promise<void> => {
  if (!item) return;
  try {
    if (item.added) {
      const existing = _deps.memoryCards.value.find((card: any) => card.word === item.word);
      if (!existing) {
        item.added = false;
        return;
      }
...
const mergeAgentMemoryCandidates = (candidates: any[] = []): void => {
  const existing = new Map(agentMemoryCandidates.value.map((item: any) => [`${item.word}-${item.reading}`, item]));
  for (const item of candidates) {
    const key = `${item.word}-${item.reading || ''}`;
    if (!item.word || existing.has(key)) continue;
    existing.set(key, { ...item, added: _deps.memoryCards.value.some((card: any) => card.word === item.word) });
  }
  agentMemoryCandidates.value = [...existing.values()].slice(0, 8);
};

// === 对话上下文 / 追问 ===
const buildAgentConversationContext = (): any[] => {
  const threadId = ensureAgentThreadId();
  const threadRuns = agentRuns.value
    .filter((run: any) => (run.threadId || threadId) === threadId)
    .slice()
    .sort((a: any, b: any) => String(a.id || '').localeCompare(String(b.id || '')))
    .slice(-4);

  const runConversation = threadRuns.flatMap((run: any) => {
    const parts: any[] = [];
  ...
  const fallbackConversation = agentMessages.value
    .filter((item: any) => (item?.role === 'user' || item?.role === 'assistant') && String(item.content || '').trim())
    .map((item: any) => ({
      role: item.role,
...
const fetchFollowUpSuggestions = async ({ message, answer }: { message: string; answer: string }): Promise<void> => {
...
const askSuggestedFollowUp = async (question: string): Promise<void> => {
  agentInput.value = question;
  await submitAgentCommand();
};

// === 例句成分解析 ===
const buildExampleComponentKey = (example: any, index: any, component: any): string => {
  const label = component?.label || '';
  const text = component?.text || '';
  return `${currentAgentRun.value?.id || 'run'}::${index}::${example?.japanese || ''}::${label}::${text}`;
};

const buildExampleSegments = (example: any): any[] => {
...
const toggleExampleComponent = (example: any, index: any, component: any): void => {
...
const getActiveExampleComponent = (example: any, index: any): any => {
  const key = `${currentAgentRun.value?.id || 'run'}::${index}::${example?.japanese || ''}`;
  const activeKey = activeExampleInspector.value[key];
  if (!activeKey) return null;
  return (example?.components || []).find((component: any) => buildExampleComponentKey(example, index, component) === activeKey) || null;
};

const isExampleComponentActive = (example: any, index: any, component: any): boolean => {
...
const explainExampleComponent = (component: any, example: any): string => {
  if (!component) return '';
  const text = String(component.text || '');
...
// === 引用展开 ===
const toggleCitation = (id: any): void => {
...
// === 回答区右侧模块导航 ===
const jumpToAgentSection = (id: string): void => {
...
const updateAgentSectionSpy = (): void => {
  agentScrollRaf = 0;
...
const loadHotPlaceholderExamples = async (force: boolean = false): Promise<void> => {
  try {
...
const currentAgentRun = computed(() => {
  ...
  return agentRuns.value.find((run: any) => run.id === activeAgentRunId.value) || agentRuns.value[agentRuns.value.length - 1];
});
...
const activeSubagentTaskDetails = computed(() => currentSubagentTasks.value.find((task: any) => task.taskId === activeSubagentTaskId.value) || null);
...
const runAgent = async (): Promise<void> => {
  ...
  const setStreamingPlaceholder = (text: string): void => {
    if (!hasTutorToken) {
  ...
  try {
    ...
    const handleEvent = (event: string, payload: any): void => {
      if (event === 'queue') {
        ...
        if (payload.activeId) {
          const activeAgent = payload.agents?.find((agent: any) => agent.id === payload.activeId);
          pushAgentEvent({
        ...
        syncAgentRun(runId, { toolCalls: agentToolCalls.value.map((call: any) => ({ ...call })) });
        pushAgentEvent({
        ...
        if (Array.isArray(payload.toolCalls) && payload.toolCalls.length > 0) {
          agentToolCalls.value = payload.toolCalls.map((call: any) => ({ ...call, status: 'done' }));
        }
        syncAgentRun(runId, {
          answer: payload.answer || assistantMessage.content || '',
          status: 'done',
          toolCalls: agentToolCalls.value.map((call: any) => ({ ...call })),
          memoryCandidates: [...agentMemoryCandidates.value],
          ...
          compactSummary: agentRuns.value.find((run: any) => run.id === runId)?.compactSummary || null,
          subagentTasks: [...(agentRuns.value.find((run: any) => run.id === runId)?.subagentTasks || [])],
...
// === 命令提交 / 停止 / 回车 ===
const submitAgentCommand = async (): Promise<void> => {
...
const stopActiveAgentRun = async (): Promise<void> => {
  const runId = activeAgentRunId.value;
...
const handleAgentEnter = async (e: KeyboardEvent): Promise<void> => {
...
const runHeroExample = (prompt: string): void => {
  agentInput.value = prompt;
  submitAgentCommand();
};

export function useAgentStream(deps: Record<string, any> = {}) {
  Object.assign(_deps, deps);

  watch(activeAgentRunId, (runId: any) => {
    if (!runId) return;
    const target = agentRuns.value.find((run: any) => run.id === runId);
    if (!target) return;
